...
news_screen.geometry("900x950")

canvas = Canvas(news_screen, width = 900, height = 950)  #add image    
...
def quick_field():#in this method we make use of selenium to extract news from website
    ...
    canvas1 = Canvas(window, width = 1500, height = 900)      
    ...
    def toPDF():#convert to pdf
        ...
        wmfile=open('flashwmark.pdf',"rb")#merging watermark with pdf
        ...
        infile.close()
        wmfile.close()
        os.startfile(r'C:\Users\hp\AppData\Local\Programs\Python\Python39\mergedQuickNewsPdf.pdf')
    def toDocs(): #convert to docs
        ...
        os.startfile(r'C:\Users\hp\AppData\Local\Programs\Python\Python39\quicknews.docx')
...
def detailed_field():#news extracted using web api called newsapi
    window = Toplevel(news_screen)
    canvas2 = Canvas(window, width = 1500, height = 900)      
    canvas2.pack(fill = "both", expand = True)    
    bg = PhotoImage(file="news2.png")      
    canvas2.create_image(500,70, anchor="nw", image=bg) 
    window.title("NEWS")
    window.geometry("1500x950")
    lab=Label(window,text ="ENTER THE NUMBER OF ARTICLES YOU WANT TO READ",width="60",height="2",font=("Calibri",20),bg="gray")
    lab.place(x=90,y=150)
    number=IntVar()
    num=Entry(window,textvariable=number,font=("Calibri",20),bg="pink")
    num.place(x = 420,y = 300,width=200,height=60)
    num.delete(0,END)
    ...
    def click(btn1,btn2,btn3):
       ...
       n=int(num.get())
       if n==0:
              mb.showwarning("showwarning","enter a valid number",parent=window)
       else:
        btn1['state']="normal"
        btn2['state']="normal"
        btn3['state']="normal"
    ...
    def toPDF():#convert to pdf
     nonlocal num
     n=num.get()
     res=requests.get('https://newsapi.org/v2/top-headlines?country=in&apiKey=23ea3b7dd1c84e7b964d9407576c6943')
     ...
     for i in range(int(n)):
        ...
        article.nlp()
        lst.append('TITLE:\n'+data[i]['title'])
        if data[i]['description']!="" or  data[i]['description']!=None:
     ...
     infile.close()
     wmfile.close()
     os.startfile(r'C:\Users\hp\AppData\Local\Programs\Python\Python39\mergednewspdf.pdf')
    def toDoc():#convert to docs
        ...
        os.startfile(r'C:\Users\hp\AppData\Local\Programs\Python\Python39\detailednews.docx')
    ...
    txt3.place(x=990,y=500)
    def browseFiles():
     fd.askopenfilenames(filetypes=[("files",".xlsx .pdf .docx")])
    subbtn = Button(window, text="SUBMIT",bg="orange", command=lambda:click(btn1,btn2,btn3),width=43,height=4)
    subbtn.place(x=360,y=430)
    explorebtn = Button(window,text = "Browse Files",bg="orange",command = browseFiles,width=30,height=4)
    explorebtn.place(x=1120,y=600)
    Button(window, text="BACK",bg="red",fg="black",activebackground = "pink",width=15, height=2,command=lambda:Main_page(window)).place(x=1370,y=0)
